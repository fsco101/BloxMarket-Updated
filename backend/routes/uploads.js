...
// Serve middleman face images
router.get('/middlemanface/:filename', (req, res) => {
  try {
    const filename = req.params.filename;
    const sanitizedFilename = path.basename(filename);
    const filepath = path.join(__dirname, '../uploads/middlemanface', sanitizedFilename);
    ...
    readStream.on('error', (err) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({ error: 'Failed to read face image file' });
      }
    });
...
router.get('/chat/:filename', (req, res) => {
  try {
    const filename = req.params.filename;
    const sanitizedFilename = path.basename(filename);
    const filepath = path.join(__dirname, '../uploads/chat', sanitizedFilename);
    ...
    readStream.on('error', (err) => {
      console.error('Error reading chat image file:', err);
      if (!res.headersSent) {
        res.status(500).json({ error: 'Failed to read chat image file' });
      }
    });
